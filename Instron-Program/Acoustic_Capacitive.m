clc;clear;
load ('terrain07_group5_duration0p5s_3.lvm');
label = 7;
fs1=22000;
...
bbb=370000;
acoustic_data = terrain07_group5_duration0p5s_3(:,2);
acoustic_data = acoustic_data(bbb:end);

capacitive_data = terrain07_group5_duration0p5s_3(:,3);
...
for i=1:size_acoustic(1)
    ...
    elseif flag==1&&y1(i)==0
        ...
        if ((en-be)>3000)
            k=k+1;
            en=min(en+8000,size_acoustic(1));
            ac_seg_data{k}=acoustic_data(be:en);
            ca_seg_data{k}=capacitive_data(be:en);
        else
            be=i+1;
        end
    end
...
%     ac_seg_data{iii}=acoustic_data(i:i+fix((bb-aa)/48/1.3));
%     ca_seg_data{iii}=capacitive_data(i:i+fix((bb-aa)/48/1.3));
%     iii=iii+1;
% end
p = size(ac_seg_data);
...
j=1;
for kn=j:2:p(2)
audio_data= ac_seg_data{kn};

noise = ac_seg_data{kn+1};
...
capacitive_wdata= ca_seg_data{kn};
